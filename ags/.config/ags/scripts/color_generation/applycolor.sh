...
apply_hyprlock() {
  ...
  # Apply colors - fix the sed pattern to properly replace template variables
  for i in "${!colorlist[@]}"; do
    # Use proper sed pattern to match {{ $varname }}
    sed -i "s|{{ \\\$${colorlist[$i]} }}|${colorvalues[$i]}|g" "$CACHE_DIR"/user/generated/hypr/hyprlock.conf
  done

  # Get current wallpaper path from file
  current_wallpaper=""
  if [ -f "$XDG_CONFIG_HOME/hypr/current_wallpaper.txt" ]; then
    current_wallpaper=$(cat "$XDG_CONFIG_HOME/hypr/current_wallpaper.txt")
  fi
  
  # Replace the placeholder with current wallpaper
  if [ -n "$current_wallpaper" ]; then
    sed -i "s|CURRENT_WALLPAPER_PATH|$current_wallpaper|g" "$CACHE_DIR"/user/generated/hypr/hyprlock.conf
  else
    # Default fallback wallpaper
    sed -i "s|CURRENT_WALLPAPER_PATH|$(xdg-user-dir PICTURES)/Wallpapers/default.jpg|g" "$CACHE_DIR"/user/generated/hypr/hyprlock.conf
  fi

  # Copy to final location
  mkdir -p "$(dirname "$XDG_CONFIG_HOME/hypr/hyprlock.conf")"
  cp "$CACHE_DIR"/user/generated/hypr/hyprlock.conf "$XDG_CONFIG_HOME"/hypr/hyprlock.conf
  
  echo "Hyprlock config updated with current colors and wallpaper."
}
