...
if args.path is not None:
    try:
        ...
        print(f"[ERROR] Decompression bomb: {args.path}", file=sys.stderr)
        sys.exit(1)
    except UnidentifiedImageError:
        print(f"[ERROR] Cannot identify image file: {args.path}", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        print(f"[ERROR] Failed to open image: {args.path}\n{e}", file=sys.stderr)
        sys.exit(1)
