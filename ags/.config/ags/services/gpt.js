...
// Function to create OpenRouter model entries
function createOpenRouterModel(name, modelId, description = "") {
    return {
        "name": name,
        "logo_name": "openrouter-symbolic",
        "description": description || `OpenRouter - ${name}`,
...
class GPTMessage extends Service {
    ...
    addDelta(delta) {
        ...
        // Filter out OpenRouter processing messages - match with or without space after colon
        if (typeof delta === 'string' && (delta.includes("OPENROUTER PROCESSING") || delta.match(/:\s*OPENROUTER\s+PROCESSING/i))) {
            return;
        }
    ...
    readResponse(stream, aiResponse) {
        ...
        stream.read_line_async(
            ...
            (stream, res) => {
                ...
                // Skip OpenRouter processing messages entirely - more robust pattern matching
                if (line && (line.includes("OPENROUTER PROCESSING") || line.match(/:\s*OPENROUTER\s+PROCESSING/i))) {
                ...
                if (line && line != '') {
                    ...
                    try {
                        ...
                        } else if (result.choices[0].delta.reasoning_content) {
                        ...
                        // Filter out OpenRouter processing messages - using regex to match variations
                        if (!(line.includes("OPENROUTER PROCESSING") || line.match(/:\s*OPENROUTER\s+PROCESSING/i))) {
