...
const pinButton = Button({
    attribute: {
        ...
        'toggle': (self) => {
            ...
            if (self.attribute.enabled) {
                // Pin mode - stay visible even when focus changes
                sideleftWindow.attribute.stayVisible = true;
                
                // If we're pinning and the window is already focused, make it unfocusable
                Utils.timeout(100, () => {
                    if (!sideleftWindow.attribute.isFocused) {
                        sideleftWindow.set_accept_focus(false);
                        sideleftWindow.set_focusable(false);
                        sideleftWindow.lower();
                    }
                });
            }
            else {
                // Normal mode - hide when focus changes
                sideleftWindow.attribute.stayVisible = false;
                // Restore focus ability
                sideleftWindow.set_accept_focus(true);
                sideleftWindow.set_focusable(true);
            }
            
            // Save the pinned state to user options
            const option = 'sidebar.keepVisible';
            updateNestedProperty(userOptions, option, self.attribute.enabled);
            execAsync(['bash', '-c', `${App.configDir}/scripts/ags/agsconfigurator.py \
                --key ${option} \
                --value ${self.attribute.enabled} \
    ...
    tooltipText: `Keep sidebar visible (${userOptions.keybinds.sidebar.pin})`,
    onClicked: (self) => self.attribute.toggle(self),
    setup: (self) => {
        setupCursorHover(self);
        // Initialize with saved state if available
        if (userOptions.sidebar.keepVisible) {
            self.attribute.enabled = true;
            self.toggleClassName('sidebar-controlbtn-enabled', true);
            Utils.timeout(100, () => {
                const sideleftWindow = App.getWindow('sideleft');
                if (sideleftWindow) {
                    sideleftWindow.attribute.stayVisible = true;
                    // Set the window to be non-focusable initially in pinned state
                    Utils.timeout(100, () => {
                        sideleftWindow.set_accept_focus(false);
                        sideleftWindow.set_focusable(false);
                        sideleftWindow.lower();
