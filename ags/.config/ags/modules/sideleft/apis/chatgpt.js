...
const ProviderSwitcher = () => {
    const ModelChoice = (id, provider) => {
        const modelSelected = MaterialIcon('check', 'norm', {
    ...
    const indicatorButton = Button({
        tooltipText: getString('Select OpenRouter Model'),
        child: Box({
            className: 'spacing-h-10 txt',
            children: [
                indicatorIcon,
        ...
        onClicked: () => {
            modelList.revealChild = !modelList.revealChild;
            indicatorChevron.label = (modelList.revealChild ? 'expand_less' : 'expand_more');
        },
        setup: setupCursorHover,
    });
    const modelList = Revealer({
        revealChild: false,
                ...
                Box({
                    ...
                    setup: (self) => self.hook(GPTService, (self) => {
                        self.children = Object.entries(GPTService.providers)
                            .map(([id, provider]) => ModelChoice(id, provider));
                    }, 'initialized'),
                })
            ]
        })
            ...
            Box({
                ...
                children: [
                    Label({
                        ...
                        wrap: true,
                        justify: Gtk.Justification.CENTER,
                        label: getString('Model selection shown above'),
                    }),
                    Button({
                        className: 'txt-subtext txt-norm icon-material',
                        label: 'info',
                        tooltipText: getString("Chat with various AI models via OpenRouter.\nNot affiliated, endorsed, or sponsored by any of the providers."),
