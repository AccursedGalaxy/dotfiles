    ...
    property var allCommands: [
        ...
        },
        {
            ...
            execute: (args, rawArgs) => {
                const keyValue = rawArgs?.trim() ?? "";
                if (args.length === 0 || args[0] === "help") {
                    Ai.addMessage(Translation.tr("Usage:\n/key YOUR_API_KEY - Set API key\n/key get - View current API key\n/key help - Show this help"), Ai.interfaceRole);
                } else if (args[0] == "get") {
                    Ai.printApiKey()
                } else if (keyValue.length === 0) {
                    Ai.addMessage(Translation.tr("Missing API key. Run `/key help` for usage."), Ai.interfaceRole);
                } else {
        ...
        {
            name: "models",
            description: Translation.tr("List available models"),
            execute: () => {
                const formatter = (modelId) => {
                    const model = Ai.models[modelId];
                    return model ? `- **${model.name}**: ${model.description}` : "";
                };

                const featuredIds = (Ai.featuredOpenRouterModels ?? []).filter(modelId => Ai.models[modelId]);
                const featuredList = featuredIds.map(formatter).filter(line => line.length > 0).join("\n");

                const remainingOpenRouter = Ai.modelList
                    .filter(modelId => modelId.startsWith("openrouter") && featuredIds.indexOf(modelId) === -1)
                    .map(formatter)
                    .filter(line => line.length > 0)
                    .join("\n");

                const otherModels = Ai.modelList
                    .filter(modelId => !modelId.startsWith("openrouter") && !modelId.includes("ollama"))
                    .map(formatter)
                    .filter(line => line.length > 0)
                    .join("\n");

                Ai.addMessage(`## Featured OpenRouter Models\n${featuredList || Translation.tr("_No featured models available_")}\n\n### More OpenRouter Options\n${remainingOpenRouter || Translation.tr("_Use /model MODEL to switch_")}\n\n### Other Providers\n${otherModels || Translation.tr("_Only OpenRouter models are loaded right now_")}`, Ai.interfaceRole);
            }
        },
        {
            name: "test",
...
`,
    ...
    function handleInput(inputText) {
        const normalizedInput = stripCommandArtifacts(inputText);
        const trimmedInput = normalizedInput.trim();
        if (trimmedInput.length === 0) return;

        if (trimmedInput.startsWith(root.commandPrefix)) {
            const spaceIndex = trimmedInput.indexOf(" ");
            const commandText = spaceIndex === -1 ? trimmedInput : trimmedInput.slice(0, spaceIndex);
            const argsString = spaceIndex === -1 ? "" : trimmedInput.slice(spaceIndex + 1);
            const sanitizedArgsString = stripCommandArtifacts(argsString);
            const args = sanitizedArgsString.length > 0 ? sanitizedArgsString.trim().split(/\s+/) : [];
            const command = commandText.substring(1).toLowerCase();
            const commandObj = root.allCommands.find(cmd => cmd.name === `${command}`);
            if (commandObj) {
                commandObj.execute(args, sanitizedArgsString);
            } else {
                Ai.addMessage(Translation.tr("Unknown command: ") + command, Ai.interfaceRole);
            }
        }
        else {
            const sanitizedUserMessage = stripCommandArtifacts(inputText);
            Ai.sendUserMessage(sanitizedUserMessage);
        }
    }
