    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            if [ -f "$thumbnail" ]; then
                ...
                # Only create restore script if not using --noswitch
            ...
            # Handle image wallpapers with swww for proper integration
            # Only set wallpaper if not using --noswitch flag
            if [[ -z "$noswitch_flag" ]]; then
                ...
                # Set image wallpaper with swww (optimized for ultrawide)
                swww img "$imgpath" \
            ...
            # Only update wallpaper path in config if not using --noswitch
            if [[ -z "$noswitch_flag" ]]; then
                set_wallpaper_path "$imgpath"
            fi
            # Always remove restore script for images to prevent video conflicts
            remove_restore
        fi
    fi
